...
-- SELECT * FROM P06_CollectionneursInfos ;


-- Cette vue donne les modeles de pieces qui ne sont pas en possession d'un collectionneur de la DB
CREATE OR REPLACE VIEW P06_PiecesNonPossedees AS
    SELECT *
    FROM P06_PieceModele
    WHERE PieceID NOT IN (
        SELECT P06_Collectionner.PieceID
        FROM P06_Collectionner
        );


-- SELECT * FROM P06_PiecesNonPossedees ;
