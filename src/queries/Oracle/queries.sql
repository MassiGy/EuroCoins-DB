...
SELECT *
FROM P06_PieceModele PM
CROSS JOIN P06_PieceTranche PT
...
FROM 
    P06_PieceModele,
    P06_PieceTranche
WHERE P06_PieceModele.PieceID = P06_PieceTranche.PieceID;
...
SELECT *
FROM P06_PieceModele WHERE PIECEID IN (
   SELECT PIECEID FROM P06_PIECEPAYS WHERE PAYSNOM LIKE 'France')
INTERSECT
SELECT *
FROM P06_PieceModele WHERE PIECEID IN (
   SELECT PIECEID FROM P06_PIECETRANCHE WHERE PIECETRANCHE LIKE 'cannelures épaisses');
...
SELECT *
FROM P06_PieceModele
WHERE PieceQuantiteFrappee < 10000000 ;
...
SELECT *
FROM P06_PieceModele
WHERE PieceID = (SELECT PieceID
                 FROM P06_PieceCaracteristique
                 WHERE PieceMateriau LIKE '%cuivré%'
                   AND PieceTaille > 100
                   AND PieceMasse > 200
...
-- trouver tous les pays qui ont produit un modèle de
-- piece suite à une competition/championnat
SELECT *
FROM P06_PiecePays
WHERE PieceID IN (
    SELECT PieceID
...
-- trouver les noms et prénoms de tous les collectionneurs
SELECT SubQuery.*
FROM (
...
-- cette requete vise à trouver tout les collectionneurs
-- qui ont une collection avec au moins 10 pièce.
SELECT * 
...
SELECT COUNT(*) NbrCollectionneur FROM P06_Collectionneur;
...
-- Donner le modèle de pièce qui a été le plus frappée
SELECT *
FROM P06_PieceModele
WHERE PieceQuantiteFrappee = (
    SELECT MAX(PieceQuantiteFrappee)
    FROM P06_PieceModele
);

-- Donner la moyenne des valeurs des pièces
SELECT AVG(PieceValeur) MoyValeur FROM P06_PieceModele;
...
FROM P06_PiecePays PP 
JOIN P06_PieceModele PM 
ON PP.PieceID = PM.PieceID
GROUP BY PaysNom;
...
-- collectionneur
SELECT C.CollectionneurID, SUM(CN.QteCollection) AS NbPiece
FROM P06_Collectionneur C
LEFT JOIN P06_Collectionner CN ON C.CollectionneurID = CN.CollectionneurID
